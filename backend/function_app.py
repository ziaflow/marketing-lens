...
def ingest_platform_data(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        if platform_id == 'GoogleSearchConsole':
            ...
            # In a real scenario, site_url comes from DB config
            data = connector.get_search_console_data("https://example.com", "2023-10-01", "2023-10-02")
            return func.HttpResponse(f"GSC Data Ingested: {len(data.get('rows', []))} rows")

        elif platform_id == 'GoogleAnalytics':
            connector = GoogleConnector(token)
            data = connector.get_ga4_report("properties/123456", "2023-10-01", "2023-10-02")
            return func.HttpResponse(f"GA4 Data Ingested: {len(data.get('rows', []))} rows")
