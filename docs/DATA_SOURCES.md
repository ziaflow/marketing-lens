...
- **APIs:**
    - **Search Console:**
        - **Search Analytics:** `searchanalytics.query` for SEO metrics (Clicks, Position).
        - **Sites:** `sites.list`, `sites.get`, `sites.add`, `sites.delete` for property management.
        - **Sitemaps:** `sitemaps.list`, `sitemaps.get`, `sitemaps.submit`, `sitemaps.delete` for sitemap operations.
        - **Inspection:** `urlInspection.index.inspect` for detailed URL indexing status.
    - **GA4 (via MCP):**
        - Uses the [Google Analytics Model Context Protocol (MCP)](https://github.com/googleanalytics/google-analytics-mcp) integration.
        - **Reporting:** `runReport` via `BetaAnalyticsDataClient`.
        - **Management:** `listAccountSummaries` via `AnalyticsAdminServiceClient`.
- **Auth:** Google OAuth 2.0 / Service Account.
- **Scopes:**
    - `https://www.googleapis.com/auth/webmasters` (Full access for Add/Delete)
